...
folder_path = 'images'
buildings_folder_path = 'images-buildings'
images = read_images_in_folder(folder_path)
images_buildings = read_images_in_folder(buildings_folder_path)
print("total buildings images taken : ",len(images_buildings))
print("total images taken : ", len(images))#printing the length of actual buildings images and total_images

img=[]
img_buildings = []
for i in range(len(images)):
    img.append(resize(color.rgb2gray(io.imread(folder_path+'/'+images[i])), (128, 128)))
for j in range(len(images_buildings)):
    img_buildings.append(resize(color.rgb2gray(io.imread(buildings_folder_path+'/'+images_buildings[j])), (128, 128))) 
#getting images from the folder
img = np.array(img)
img_buildings = np.array(img_buildings)
...
mag_buildings = np.zeros(((len(images_buildings),128,128)))
theta_buildings = np.zeros(((len(images_buildings),128,128)))

mag_f,theta_f = mag_and_angle_cal(mag,theta,img)
mag_buildings_f,theta_buildings_f = mag_and_angle_cal(mag_buildings,theta_buildings,img_buildings)

#calling the function created above to get the magnitude and theta for total and buildings images
...
hist_buildings = histogram(mag_buildings_f,theta_buildings_f)
...
feature_vecs_buildings= features(hist_buildings) #getting feature vectors for buildings images and total images

print(f'Number of HOG features = {len(feature_vecs[0]) * len(feature_vecs[0][0]) * len(feature_vecs[0][0][0])}')
print(f'Number of HOG features in buildings images = {len(feature_vecs_buildings[0]) * len(feature_vecs_buildings[0][0]) * len(feature_vecs_buildings[0][0][0])}')

Hog_features = len(feature_vecs[0]) * len(feature_vecs[0][0]) * len(feature_vecs[0][0][0])
Hog_features_buildings= len(feature_vecs_buildings[0]) * len(feature_vecs_buildings[0][0]) * len(feature_vecs_buildings[0][0][0])
...
for j in range(len(images_buildings)):
    feature_vecs_buildings[j] = np.array(feature_vecs_buildings[j]).ravel()
#flattening the featur_vecs array


feature_vecs_arr = np.zeros(( len(images), Hog_features))
feature_vecs_buildings_arr = np.zeros(( len(images_buildings), Hog_features_buildings ))

for i in range(len(feature_vecs)):
    for j in range(len(feature_vecs[0])):
        feature_vecs_arr[i][j]  += feature_vecs[i][j]
for i in range(len(feature_vecs_buildings)):
    for j in range(len(feature_vecs_buildings[0])):
        feature_vecs_buildings_arr[i][j] += feature_vecs_buildings[i][j]
#forming 2d array of feature vectors for actual buildings images and total images


y_minus = np.zeros(len(images)) - 1
y_plus = np.zeros(len(images_buildings)) + 1
y_minus = y_minus.T
y_minus = y_minus.reshape((len(images),1))
labelled_1 = np.hstack((feature_vecs_arr,y_minus))
y_plus = y_plus.reshape((len(images_buildings),1))
labelled_2 = np.hstack((feature_vecs_buildings_arr,y_plus))
labelled = np.vstack((labelled_1,labelled_2))
np.random.shuffle(labelled)
#labelling the actual buildings images as 1 and rest as -1 and then making a shuffled input data 
